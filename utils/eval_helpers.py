...
def align_sim3(model, data):
    """
    Umeyama Sim3 alignment: solve s,R,t such that data ~= s*R*model + t
    Inputs are 3xN numpy arrays.
    """
    if model.shape != data.shape or model.shape[0] != 3:
        raise ValueError(f"Expected 3xN arrays with same shape, got model={model.shape} data={data.shape}")
    n = model.shape[1]
    if n < 3:
        raise ValueError(f"Need at least 3 points for Sim3 alignment, got N={n}")

    mu_m = model.mean(1).reshape((3, -1))
    mu_d = data.mean(1).reshape((3, -1))
    X = model - mu_m
    Y = data - mu_d
    cov = (Y @ X.T) / float(n)
    ...
    if (np.linalg.det(U) * np.linalg.det(Vh) < 0):
        S[2, 2] = -1
    R = U @ S @ Vh
    var_m = (X * X).sum() / float(n)
    if var_m <= 1e-12:
        raise ValueError("Degenerate Sim3: variance too small.")
    scale = float(np.trace(np.diag(D) @ S) / var_m)
    t = mu_d - scale * (R @ mu_m)
    return scale, R, t
...
def eval_save(
    ...
    add_new_gaussians,
    save_renders=True,
    pose_eval: dict | None = None,
    transient_cfg: dict | None = None,
):
    # timer = Timer()
    # timer.start()
    split = False
    ...
    if save_renders:
        ...
        render_rgb_static_dir = None
        render_rgb_transient_dir = None
        if isinstance(transient_cfg, dict) and bool(transient_cfg.get("enabled", False)):
            render_rgb_static_dir = os.path.join(eval_dir, "color_static")
            render_rgb_transient_dir = os.path.join(eval_dir, "color_transient")
            os.makedirs(render_rgb_static_dir, exist_ok=True)
            os.makedirs(render_rgb_transient_dir, exist_ok=True)

    gt_w2c_list = []
    gt_position_list = []
    est_position_list = []
    # Use absolute poses to build a consistent GT trajectory across C3VD train/test splits.
    # Both train and test subsets store absolute poses in `.poses`, but `get_pose()` is relative-to-subset,
    # which causes jumps when you interleave them.
    global_pose0_abs = None
    ...
    def get_prev_train_time_idx(total_time_idx: int) -> int | None:
        """
        For split datasets (C3VD 7::8), transient params are only trained/stored for train frames.
        For test-frame rendering/metrics, we optionally reuse the most recent train transient slice.
        Returns the *train-subset* time_idx, or None if none exists.
        """
        for t in range(int(total_time_idx) - 1, -1, -1):
            dtype, _ds, tidx = get_idx(t)
            if dtype == "train":
                return int(tidx)
        return None
    ...
    for total_time_idx in tqdm(range(num_frames)):
        ...
        # Lazily pick global reference as the absolute pose of the very first frame in the full sequence.
        if global_pose0_abs is None:
            if split:
                global_pose0_abs = train_dataset.poses[0].detach().cpu()
            else:
                global_pose0_abs = dataset.poses[0].detach().cpu()
        pose_abs = dataset.poses[time_idx].detach().cpu()
        pose_rel = relative_transformation(global_pose0_abs, pose_abs, orthogonal_rotations=False)
        gt_w2c_list.append(torch.linalg.inv(pose_rel))
    ...
    if split:
        ...
        # Align GT test poses into the estimated coordinate system for NVS rendering.
        # For non-metric pipelines (e.g., pred-depth), use Sim3 so global scale mismatch doesn't break eval renders.
        align_mode = "se3"
        align_R = None
        if isinstance(pose_eval, dict):
            align_mode = str(pose_eval.get("align_mode", pose_eval.get("align", "se3"))).lower()
        gt_train = torch.stack(gt_position_list)[train_idx].detach().cpu().numpy().T
        est_train = torch.stack(est_position_list).detach().cpu().numpy().T
        if align_mode == "sim3":
            s, R, t = align_sim3(gt_train, est_train)
            align_R = np.asarray(R, dtype=np.float32)
            horn_gt_position = [s * (R @ gt_position.numpy()) + t.squeeze() for gt_position in gt_position_list]
        elif align_mode == "none":
            horn_gt_position = [gt_position.numpy() for gt_position in gt_position_list]
        else:
            R, t, _ = align(gt_train, est_train)
            align_R = np.asarray(R, dtype=np.float32)
            horn_gt_position = [R @ gt_position.numpy() + t.squeeze() for gt_position in gt_position_list]
    ...
    for total_time_idx in tqdm(range(num_frames)):
        ...
        if dataset_type == 'train':
            ...
            curr_rel_w2c = torch.eye(4).cuda().float()
            curr_rel_w2c[:3, :3] = build_rotation(cam_rot)
            curr_rel_w2c[:3, 3] = cam_tran
            transformed_pts = transform_to_frame_eval(final_params, (cam_rot, cam_tran))
        else:
            w2c = gt_w2c_list[total_time_idx].clone()
            if split and align_mode != "none" and align_R is not None:
                w2c[:3, :3] = torch.tensor(align_R, dtype=w2c.dtype) @ w2c[:3, :3]
            w2c[:3, 3] = torch.Tensor(horn_gt_position[total_time_idx])
            curr_rel_w2c = w2c.cuda()
            transformed_pts = transform_to_frame_eval(final_params, rel_w2c=w2c.cuda()) # use aligned GT pose to render
            
        # Define current frame data
        curr_data = {'cam': cam, 'im': color, 'depth': depth, 'id': time_idx, 'intrinsics': intrinsics, 'w2c': first_frame_w2c}

        # When transient decomposition is enabled, some configs want to render *all* frames (train+test)
        # for visualization. This is expensive; allow configs to override and render only test frames
        # during evaluation (speed), then re-render all frames later when making videos.
        #
        # Backward-compat: if transient is enabled and no explicit knob is provided, keep the old
        # behavior (render all frames).
        render_all_frames = False
        if isinstance(transient_cfg, dict) and bool(transient_cfg.get("enabled", False)):
            if "render_all_frames" in transient_cfg:
                render_all_frames = bool(transient_cfg.get("render_all_frames", False))
            else:
                render_all_frames = True  # legacy default for transient debug

        # For split datasets (C3VD train/test), optionally skip rendering train frames.
        if split and (not render_all_frames) and dataset_type == "train":
            continue
        ...
        im, _, _= Renderer(raster_settings=curr_data['cam'])(**rendervar)
        im_static = torch.clamp(im, 0.0, 1.0)

        # Optional transient rendering + blend (DeSplat-style: transient overlays static).
        im_transient = None
        alpha_t = None
        if (
            ...
            apply_to_test = str(transient_cfg.get("apply_to_test_frames", "none")).lower()
            t_time_idx = None
            if n_per > 0:
                if dataset_type == "train":
                    t_time_idx = int(time_idx)
                elif apply_to_test in ("prev_train", "previous_train", "last_train"):
                    t_time_idx = get_prev_train_time_idx(total_time_idx)

            if t_time_idx is not None and n_per > 0 and curr_rel_w2c is not None:
                sl0 = int(t_time_idx) * int(n_per)
                sl1 = sl0 + int(n_per)
                if int(final_params["t_means3D"].shape[0]) >= sl1:
                    tparams = {
                        "means3D": final_params["t_means3D"][sl0:sl1],
                        "rgb_colors": final_params["t_rgb_colors"][sl0:sl1],
                        "unnorm_rotations": final_params["t_unnorm_rotations"][sl0:sl1],
                        "logit_opacities": final_params["t_logit_opacities"][sl0:sl1],
                        "log_scales": final_params["t_log_scales"][sl0:sl1],
                    }

                    t_transformed_pts = transform_to_frame_eval(
                        {"means3D": tparams["means3D"]}, rel_w2c=curr_rel_w2c
                    )
                    t_rendervar = transformed_params2rendervar(tparams, t_transformed_pts)
                    im_t, _, _ = Renderer(raster_settings=curr_data["cam"])(**t_rendervar)
                    im_transient = torch.clamp(im_t, 0.0, 1.0)

                    t_depth_sil_rendervar = transformed_params2depthplussilhouette(
                        tparams, curr_data["w2c"], t_transformed_pts
                    )
                    t_depth_sil, _, _ = Renderer(raster_settings=curr_data["cam"])(**t_depth_sil_rendervar)
                    alpha_t = t_depth_sil[1, :, :].clamp(0.0, 1.0)

        if im_transient is not None and isinstance(alpha_t, torch.Tensor):
            im_blend = (im_transient + (1.0 - alpha_t.unsqueeze(0)) * im_static).clamp(0.0, 1.0)
        else:
            im_blend = im_static
        ...
        # else:
        weighted_im = im_blend * valid_depth_mask
        ...
        if save_renders:
            ...
            viz_render_im = torch.clamp(im_blend, 0, 1)
            ...
            if render_rgb_static_dir is not None:
                save_im_s = im_static.detach().cpu().permute(1, 2, 0).numpy()
                save_im_s = np.clip(save_im_s * 255, 0, 255).astype(np.uint8)
                cv2.imwrite(os.path.join(render_rgb_static_dir, "color_{:04d}.png".format(total_time_idx)), cv2.cvtColor(save_im_s, cv2.COLOR_RGB2BGR))
            if render_rgb_transient_dir is not None:
                if im_transient is None:
                    im_t_vis = torch.zeros_like(im_static)
                else:
                    im_t_vis = im_transient
                save_im_t = im_t_vis.detach().cpu().permute(1, 2, 0).numpy()
                save_im_t = np.clip(save_im_t * 255, 0, 255).astype(np.uint8)
                cv2.imwrite(os.path.join(render_rgb_transient_dir, "color_{:04d}.png".format(total_time_idx)), cv2.cvtColor(save_im_t, cv2.COLOR_RGB2BGR))
        ...
        plot_rgbd_silhouette(color, depth, im_blend, rastered_depth_viz, presence_sil_mask, diff_depth_l1,
    ...
    if split:
        ...
        with open(os.path.join(eval_dir, 'est_w2c.txt'), 'w') as f:
            # Match gt_train_w2c ordering (train_idx in total-time coordinates).
            # total_time_idx -> train_dataset time_idx mapping is the same as used above in get_idx().
            for total_time_idx in train_idx:
                _dataset_type, _dataset, time_idx = get_idx(total_time_idx)
                assert _dataset_type == "train"
                tensor = latest_est_w2c_list[time_idx]
                line = ''
