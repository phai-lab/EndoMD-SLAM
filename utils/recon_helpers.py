...
def setup_camera(w, h, k, w2c, near=0.01, far=100, bg=[0,0,0], use_simplification=True, debug=False):
    ...
    # `diff_gaussian_rasterization` has multiple ABI variants in the wild:
    # some builds require `debug`, others reject it.
    try:
        return Camera(**kwargs, debug=bool(debug))
    except TypeError:
