...
def remove_points(to_remove, params, variables, optimizer):
    ...
    for k, v in params.items():
        ...
        if not isinstance(v, torch.Tensor) or v.ndim == 0:
            continue
        # Only apply the static-gaussian mask to tensors with matching leading dimension.
        # This keeps additional parameter tensors (e.g., per-frame transient layers) intact.
        if int(v.shape[0]) != n:
            continue
        keys.append(k)
